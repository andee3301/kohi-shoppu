    ...
    <section class="p-4 mt-6 px-2 py-2 lg:px-24">
        <div class="bg-surface rounded-xl text-on-surface-600 p-4 lg:p-10 shadow-lg">
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
                <div class="col-span-2 grid grid-cols-1 gap-6 lg:border-r-2 lg:border-r-on-surface-200">
                    ...
                    <div class="flex flex-col">
                        ...
                        <span class="text-base">
                            ...
                            <span class="text-on-surface-500 text-xs">({{ $order->created_at->diffForHumans() }})</span>
                        </span>
                    </div>

                    <div class="flex flex-col">
                        <h2 class="text-xs font-bold text-on-surface-500 uppercase">{{ __('Status') }}</h2>
                        <span class="text-base">{{ ucfirst($order->status) }}</span>
                    </div>
                </div>
            ...
            <div class="relative overflow-x-auto mt-10 mb-10">
                <table class="w-full text-left table-auto">
                    <thead class="font-bold text-on-surface-800 uppercase bg-on-surface-200">
                        <tr>
                            <th scope="col" class="text-xs font-bold px-2 py-3">{{ __('Product name') }}</th>
                            <th scope="col" class="text-xs font-bold px-2 py-3">{{ __('Notes') }}</th>
                            <th scope="col" class="text-xs font-bold px-2 py-3">{{ __('Unit price incl. VAT') }}</th>
                            <th scope="col" class="text-xs font-bold px-2 py-3">{{ __('Quantity') }}</th>
                            <th scope="col" class="text-xs font-bold px-2 py-3">{{ __('Total') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($order->cart->items as $item)
                            <tr class="border-b-2 border-on-surface-100">
                                <th class="text-base px-2 py-4 font-normal">{{ $item->product->name }}</th>
                                <td class="text-base px-2 py-4">{{ $item->notes }}</td>
                                <td class="text-base px-2 py-4">
                                    {{ number_format($item->product->price) }} {{ $item->product->currency }}
                                </td>
                                <td class="text-base px-2 py-4">{{ $item->quantity }}</td>
                                <td class="text-base px-2 py-4">
            ...
            <div class="flex flex-row justify-end rounded-lg bg-surface">
                <ul class="w-full lg:w-2/5">
                    ...
                    <li class="flex flex-row items-center justify-between py-2">
                        ...
                        <span class="text-base lowercase">{{ __('Free') }}</span>
                    </li>
                    <li class="flex flex-row items-center justify-between py-2">
